...
- La sección "Lo que dicen nuestros clientes en Google" (`GoogleReviews` en `pages/home.jsx`) funciona igual, pero con el widget [Elfsight Google Reviews](https://elfsight.com/google-reviews-widget/): añade ese widget en el mismo panel de Elfsight, conecta la ficha de Google de ENMAR, y pega el ID en `ELFSIGHT_GOOGLE_WIDGET_ID`. Mientras esté vacío se muestra un aviso invitando a dejar la primera reseña, con enlace directo a la ficha de Google (`GOOGLE_REVIEW_URL` en `assets/shared.jsx`). Los testimonios inventados que traía el proyecto originalmente se han retirado — no debe mostrarse una web real con citas de clientes que no existen.
