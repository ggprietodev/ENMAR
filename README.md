...
- La home incluye una sección "Síguenos en Instagram" (`InstagramFeed` en `pages/home.jsx`) y otra "Lo que dicen nuestros clientes en Google" (`GoogleReviews`), ambas con los widgets gratuitos de [SociableKit](https://www.sociablekit.com/) (plan "forever free", sin tarjeta), conectados a @grupoenmar y a la ficha de Google de ENMAR desde su panel respectivo. Los scripts (`https://widgets.sociablekit.com/instagram-feed/widget.js` y `.../google-reviews/widget.js`) se cargan en `index.html`; los `data-embed-id` de cada `<div>` se gestionan desde el panel de SociableKit — si algún día hay que reconectar o cambiar de cuenta, se genera un `data-embed-id` nuevo allí y se sustituye en `pages/home.jsx`. También hay un enlace "Danos tu opinión en Google" (`GOOGLE_REVIEW_URL` en `assets/shared.jsx`) en el footer y en contacto. Los testimonios inventados que traía el proyecto originalmente se han retirado — no debe mostrarse una web real con citas de clientes que no existen.
